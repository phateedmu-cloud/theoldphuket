...
export default function RoomsSelection() {
  ...
  // ฟังก์ชันหาโฟลเดอร์รูปภาพและคำนวณราคาเฉพาะห้อง
  const getRoomMeta = (roomName, baseMarketPrice) => {
    let adjustment = 0;
    let folder = "deluxe";
    const name = roomName.toLowerCase();
    
    if (name.includes('pool access')) { adjustment = 1500; folder = "deluxe-pool-access"; }
    else if (name.includes('family')) { adjustment = 2500; folder = "family-deluxe"; }
    else if (name.includes('pool view')) { adjustment = 700; folder = "deluxe-pool-view"; }
    else if (name.includes('terrace')) { adjustment = 500; folder = "deluxe-terrace"; }
    else if (name.includes('jacuzzi')) { adjustment = 2000; folder = "jacuzzi-deluxe"; }
    else { folder = "deluxe"; }

    const marketPrice = baseMarketPrice + adjustment;
    const ourPrice = Math.floor(marketPrice * 0.9);
    
    // สร้าง Array รูปภาพ 01-10.jpg
    const images = Array.from({ length: 10 }, (_, i) => `/images/rooms/${folder}/${String(i + 1).padStart(2, '0')}.jpg`);
    
    return { marketPrice, ourPrice, discount: marketPrice - ourPrice, images };
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white">
        ...
        <p className="font-serif italic text-gray-400 animate-pulse uppercase tracking-[0.3em] text-[10px]">AI Matching Market Rates...</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-gray-50 min-h-screen pb-20 font-sans antialiased">
      ...
      {/* ✅ แก้ไข: เพิ่ม Padding Top (pt-24) เพื่อไม่ให้ Navbar บัง Header */}
      <div className="pt-24 bg-gray-900 text-white pb-16 px-6 relative overflow-hidden">
        <div className="max-w-6xl mx-auto relative z-10 flex flex-col md:flex-row justify-between items-center">
          <div className="text-center md:text-left">
            <h2 className="text-4xl md:text-5xl font-serif font-bold tracking-tight uppercase italic mb-4">Choose Your Sanctuary</h2>
          ...
          <div className="flex items-center gap-4 bg-white/5 backdrop-blur-md border border-white/10 p-4 rounded-sm shadow-2xl mt-8 md:mt-0">
            ...
            <span className="text-[10px] uppercase tracking-widest font-bold text-[#E5C595]">AI Price Match Active</span>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto mt-12 px-4 space-y-12">
        {availableRooms.length === 0 ? (
          <div className="text-center py-32 bg-white shadow-xl rounded-sm">
            <p className="text-gray-400 font-serif italic text-lg uppercase tracking-widest">No rooms available for these dates.</p>
          </div>
        ) : (
            ...
            return (
              <div key={room.id} className="bg-white shadow-2xl overflow-hidden flex flex-col md:flex-row border border-gray-100 group transition-all duration-500">
                ...
                <div className="md:w-2/5 relative h-80 md:h-auto overflow-hidden">
                  ...
                  <div className="absolute top-6 left-6 z-10 bg-gray-900/90 text-[#E5C595] px-4 py-2 text-[10px] font-bold uppercase tracking-widest backdrop-blur-sm">Official Rate</div>
                </div>
                ...
                <div className="md:w-3/5 p-8 md:p-12 flex flex-col justify-between">
                  <div>
                    ...
                    <p className="text-gray-400 font-light text-sm italic border-l-2 border-[#E5C595] pl-6 mb-10 leading-relaxed">
                      {room.description || "A masterfully crafted space blending Phuket's rich heritage with contemporary elegance."}
                    </p>
                  </div>
                  ...
                  <div className="pt-8 border-t border-gray-50 flex flex-col md:flex-row md:items-end justify-between gap-8">
                    <div className="space-y-3">
                      <div className="flex items-center gap-3">
                        <span className="text-[10px] uppercase tracking-widest text-gray-300 line-through">Market: THB {meta.marketPrice.toLocaleString()}</span>
                        <span className="bg-green-50 text-green-600 text-[9px] font-bold px-2 py-0.5 rounded-full uppercase">Save THB {meta.discount.toLocaleString()}</span>
                      </div>
                      <div className="flex items-baseline gap-2">
                        ...
                        <span className="text-[10px] text-gray-400 uppercase tracking-widest font-medium">/ Night</span>
                      </div>
                    </div>
                    <button 
                      type="button"
